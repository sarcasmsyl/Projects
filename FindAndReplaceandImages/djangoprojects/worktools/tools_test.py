...
def FindandReplace(exceldoc):
    ...
    # Save the input exceldoc to FILE_DIR
    input_path = FILE_DIR / exceldoc.name

    # Handle InMemoryUploadedFile or TemporaryUploadedFile correctly
    with open(input_path, 'wb') as f:
        for chunk in exceldoc.chunks():
            f.write(chunk)

    # Add what you want to find and replace in this dictionary
    find_and_replace = {
        '”': '"',
        '“': '"',
        "─": "─",
    ...
    cleaned_path = FILE_DIR / f"{input_path.stem}_Cleaned.xlsx"
    wb.save(cleaned_path)
    
    return cleaned_path

def FindandReplace2(exceldoc):
    FILE_DIR = Path(settings.MEDIA_ROOT) / "FindAndReplace"
    FILE_DIR.mkdir(parents=True, exist_ok=True)
...
def zip_directory(directory_path):
    zf = zipfile.ZipFile(f'{directory_path}.zip', "w")
    zip_path = directory_path.with_suffix('.zip')
    for dirname, subdirs, files in os.walk(f'{directory_path}'):
        zf.write(dirname)
        for filename in files:
            zf.write(os.path.join(dirname, filename))
    zf.close()
    shutil.make_archive(zip_path.stem, 'zip', directory_path)
    return zip_path